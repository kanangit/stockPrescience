# This script is adapted from Libby Hemphill's get-nytimes-articles.py script
# CASM Lab -- https://github.com/casmlab/get-nytimes-articles
# The request_string line in this script has been changed from the original
# (a) to allow for a "fq" filtered query search (e.g., setting the "query"
# fed in from the script's accompanying config.sys file to look for collocates
# and (b) to restrict the search by the NYT's source and news_desk fields
# [change the latter two fields as needed in the request_string lines]
...
def getArticles(date, query, api_key, json_file_path):
    ...
    for page in range(101):
        try:
            request_string = "http://api.nytimes.com/svc/search/v2/articlesearch.json?fq=%28NOT+news_desk.contains%3A%28%22Sports%22+%22Classified%22%29%29+AND+%28source%3A%28%22The+New+York+Times%22%29%29+AND+" + query + "&begin_date=" + date + "&end_date=" + date + "&page=" + str(page) + "&api-key=" + api_key
