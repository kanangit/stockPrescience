...
start_day = '20180101'
end_day = '20180110'
filename = r'D:\kananovich\personal\OneDrive\Documents\family\jobsAnton\dataIncubator\stage02\myproject\data\scrapped\final\dumpFinancialSoupNoCroco20180101-20180110.csv'

def remCroco(record):
	recDeleted = record	
	match = r'([^a-zA-Z|\'|\-])'
	recDeleted = re.sub(match,' ',record)			
	recDeleted = re.sub('    ',' ',record)
	recDeleted = re.sub('   ',' ',record)
	recDeleted = re.sub('  ',' ',record)
	return recDeleted
    ...
    while day != end_day:
        ...
        for i in range(0,1):
            ...
            try:
                ...
                articles = api.search(q = {searchword},
...
def scarp_articles_text(articles_df):
    ...
    for j in range(0, len(articles_df)):
        ...
        print(url)
        req = session.get(url)
        huyup = BeautifulSoup(req.text, 'lxml') 
        soup = huyup.find('section', attrs={'name':'articleBody'})

        #initiate non-existing tag
        #we will need it later to check if somothing was found
        i_dont_exist = huyup.find('nonExistantTag')
        notype = type(i_dont_exist)
        ...
        if paragraph_tags == []:
            paragraph_tags = huyup.find_all('p', itemprop = 'articleBody')
...
export_csv = articles_df.to_csv (filename, index = None, header=True) 